...
import ai.certifai.solution.object_detection.AvocadoBananaDetector.AvocadoBananaDetector_YOLOv2;
...
public class Project_ModelTrain {
    private static final Logger log = LoggerFactory.getLogger(Project_ModelTrain.class);
    ...
    private static File modelFilename = new File(System.getProperty("user.dir"), "cdle-traininglabs-main/generated-models/OrangeBananaDetector_yolov2.zip");
    ...
    private static Scalar[] colormap1 = {RED,RED};

    public static void main(String[] args) throws Exception {

        // Create iterators
        Project_Iterator.setup();
        RecordReaderDataSetIterator trainIter = Project_Iterator.trainIterator(batchSize);
        RecordReaderDataSetIterator testIter = Project_Iterator.testIterator(1);
        ...
        if (modelFilename.exists()) {
            ...
            System.out.println(model.summary(InputType.convolutional(
                    Project_Iterator.yoloheight,
                    Project_Iterator.yolowidth,
                    nClasses)));
            ...
            EarlyStoppingConfiguration esConfig= new EarlyStoppingConfiguration.Builder()
                    ...
                    .build();
            EarlyStoppingGraphTrainer trainer= new EarlyStoppingGraphTrainer(esConfig,model,trainIter);
    ...
    private static ComputationGraph getComputationGraph(ComputationGraph pretrained, INDArray priors, FineTuneConfiguration fineTuneConf) {
        ...
        return new TransferLearning.GraphBuilder(pretrained)
                ...
                .setFeatureExtractor("conv2d_19")
                .removeVertexKeepConnections("conv2d_23")
                .removeVertexKeepConnections("outputs")
                .addLayer("conv2d_23_new",
                ...
                thread = new Thread(() ->
                {
                    while (frame != null) {
                        try {
                            ...
                            resize(rawImage, resizeImage, new Size(Project_Iterator.yolowidth, Project_Iterator.yoloheight));
    ...
    private static Mat drawResults(List<DetectedObject> objects, Mat mat, int w, int h) {
        ...
        for (DetectedObject obj1 : objects) {
            String label1 = labels.get(obj1.getPredictedClass());
            if (label1.equals("orange"))
                alerted = false;
        }
        ...
        for (DetectedObject obj : objects) {
            ...
            int x1 = (int) Math.round(w * xy1[0] / Project_Iterator.gridWidth);
            int y1 = (int) Math.round(h * xy1[1] / Project_Iterator.gridHeight);
            int x2 = (int) Math.round(w * xy2[0] / Project_Iterator.gridWidth);
            int y2 = (int) Math.round(h * xy2[1] / Project_Iterator.gridHeight);
            if (alerted) {
                //Drawing bounding box
                rectangle(mat, new Point(x1, y1), new Point(x2, y2), colormap1[obj.getPredictedClass()], 2, 0, 0);
                //Displaying label text
                labeltext = label + " " + String.format("%.2f", obj.getConfidence() * 100) + "%";
                int[] baseline = {0};
                Size textSize = getTextSize(labeltext, FONT_HERSHEY_DUPLEX, 1, 1, baseline);
                rectangle(mat, new Point(x1 + 2, y2 - 2), new Point(x1 + 2 + textSize.get(0), y2 - 2 - textSize.get(1)), colormap1[obj.getPredictedClass()], FILLED, 0, 0);
